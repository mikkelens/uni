...
public class ClosestBall {
    public int computeClosest(ArrayList<Integer> players, ArrayList<Integer> balls) {
        ...
        int b = 0; // reused with offset between players
        for (int p = 0; p < players.size(); p++) {
            int player = players.get(p);
//            System.err.println("Player " + player);
            int difference = Math.abs(balls.get(b) - player);
            while (b + 1 < balls.size()) {
                int next_difference = Math.abs(balls.get(b + 1) - player);
            ...
            if (difference < smallest) {
                smallest = difference;
//                System.err.println("New smallest difference: " + difference);
            }
        }
        return smallest;
    }
